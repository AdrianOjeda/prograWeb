            ...
            <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                @foreach($alumnos as $alumno)
                    <tr class="text-gray-700 dark:text-gray-400 odd:bg-gray-50 even:bg-white">
                        <td class="px-4 py-3">{{ $alumno->id }}</td>
                        <td class="px-4 py-3">
                            <a href="{{ route('alumnos.show', $alumno) }}" class="text-blue-600 hover:underline">
                                {{ $alumno->nombre }} {{ $alumno->apellido }}
                            </a>
                        </td>
                        <td class="px-4 py-3">{{ $alumno->codigo }}</td>
                        <td class="px-4 py-3">
                        ...
                        <td class="px-4 py-3">{{ $alumno->created_at->format('d/m/Y') }}</td>
                        <td class="px-4 py-3">{{ $alumno->updated_at->format('d/m/Y') }}</td>
                        <td class="px-4 py-3 flex space-x-4">
                            <!-- Botón para editar -->
                            <a href="{{ route('alumnos.edit', $alumno) }}" class="flex items-center px-3 py-2 text-sm font-medium text-blue-600 bg-blue-100 rounded-lg hover:bg-blue-200 transition-colors duration-150">
